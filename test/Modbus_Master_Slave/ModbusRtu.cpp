 ...
 *
 * @description
 *  Arduino library for communicating with Modbus_RTU devices
 *  over RS232/USB/485 via RTU protocol.
 *
 *  Further information:
 *  http://modbus.org/
 *  http://modbus.org/docs/Modbus_RTU_over_serial_line_V1_02.pdf
 *
 * @license
 ...
 *
 * @defgroup setup Modbus_RTU Object Instantiation/Initialization
 * @defgroup loop Modbus_RTU Object Management
 * @defgroup buffer Modbus_RTU Buffer Management
 * @defgroup discrete Modbus_RTU Function Codes for Discrete Coils/Inputs
 * @defgroup register Modbus_RTU Function Codes for Holding/Input Registers
 *
 */
 ...
 *
 * Modbus_RTURtu needs a pin for flow control only for RS485 mode. Pins 0 and 1
 * cannot be used.
 *
 * First call begin() on your serial port, and then start up Modbus_RTURtu by
 ...
 * @ingroup setup
 */
void Modbus_RTU::Modbus_Init(uint8_t u8id, Stream& port, uint8_t u8txenpin)
{
    this->port = &port;
    this->u8id = u8id;
 ...
 * USE Modbus_RTU(uint8_t, T_Stream&, uint8_t) INSTEAD.
 *
 * @param u8id   node address 0=master, 1..247=slave
 * @param u8serno  serial port used 0..3 (ignored for software serial)
 * @param u8txenpin pin for txen RS-485 (=0 means USB/RS232C mode)
 * @ingroup setup
 * @overload Modbus_RTU::Modbus_RTU(uint8_t u8id, T_Stream& port, uint8_t u8txenpin)
 */
void Modbus_RTU::Modbus_Init1(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin)
{
    this->u8id = u8id;
    this->u8txenpin = u8txenpin;
    this->u16timeOut = 1000;
 ...
 * Modbus_RTURtu's own begin() functions.)
 *
 * @ingroup setup
 */
void Modbus_RTU::start()
{
...
/**
 * @brief
 * DEPRECATED Install a serial port, begin() it, and start Modbus_RTURtu.
 *
 * ONLY PROVIDED FOR BACKWARDS COMPATIBILITY.
 * USE Serial.begin(<baud rate>); FOLLOWED BY Modbus_RTU.start() INSTEAD.
 *
...
void Modbus_RTU::begin(T_Stream* install_port, long u32speed)
{
    port = install_port;
 ...
 */
template<typename T_Stream>
void Modbus_RTU::begin(T_Stream* install_port, long u32speed, uint8_t u8txenpin)
{
    this->u8txenpin = u8txenpin;
...
/**
 * @brief
 * DEPRECATED. begin() hardware serial port and start Modbus_RTURtu.
 *
 * ONLY PROVIDED FOR BACKWARDS COMPATIBILITY.
 * USE Serial.begin(<baud rate>); FOLLOWED BY Modbus_RTU.start() INSTEAD.
 *
 ...
 * @ingroup setup
 */
void Modbus_RTU::begin(long u32speed)
{
 ...
 * @ingroup setup
 */
void Modbus_RTU::setID( uint8_t u8id)
{
    if (( u8id != 0) && (u8id <= 247))
    {
        this->u8id = u8id;
 ...
 * @ingroup setup
 */
void Modbus_RTU::setTxendPinOverTime( uint32_t u32overTime )
{
 ...
 * @ingroup setup
 */
void Modbus_RTU::setTimeOut( uint16_t u16timeOut)
{
 ...
 * @ingroup loop
 */
boolean Modbus_RTU::getTimeOutState()
{
 ...
 * @return   EXC_ADDR_RANGE = 2  Address beyond available space for Modbus_RTU registers
 * @return   EXC_REGS_QUANT = 3  Coils or registers number beyond the available space
 * @ingroup buffer
 */
uint8_t Modbus_RTU::getLastError()
{
    return u8lastError;
}
 ...
 */
int8_t Modbus_RTU::query( modbus_t telegram )
{
...
/**
 * @brief *** Only for Modbus_RTU Master ***
 ...
 * @ingroup loop
 */
int8_t Modbus_RTU::poll( uint16_t *regs, uint8_t u8size )
{
...
/**
 * @brief
 * This method moves Serial buffer data to the Modbus_RTU au8Buffer.
 *
 * @return buffer size if OK, ERR_BUFF_OVERFLOW if u8BufferSize >= MAX_BUFFER
 * @ingroup buffer
 */
int8_t Modbus_RTU::getRxBuffer()
{
 ...
 * @ingroup buffer
 */
uint16_t Modbus_RTU::calcCRC(uint8_t u8length)
{
 ...
 * @ingroup buffer
 */
uint8_t Modbus_RTU::validateRequest()
{
 ...
 * @ingroup buffer
 */
uint8_t Modbus_RTU::validateAnswer()
{
    // check message crc vs calculated crc
    uint16_t u16MsgCRC =
        ((au8Buffer[u8BufferSize - 2] << 8)
 ...
 * @ingroup buffer
 */
void Modbus_RTU::buildException( uint8_t u8exception )
{
 ...
 * @ingroup discrete
 */
int8_t Modbus_RTU::process_FC1( uint16_t *regs, uint8_t /*u8size*/ )
{
 ...
 * @ingroup register
 */
int8_t Modbus_RTU::process_FC3( uint16_t *regs, uint8_t /*u8size*/ )
{
 ...
 * @ingroup discrete
 */
int8_t Modbus_RTU::process_FC5( uint16_t *regs, uint8_t /*u8size*/ )
{
 ...
 * @ingroup register
 */
int8_t Modbus_RTU::process_FC6( uint16_t *regs, uint8_t /*u8size*/ )
{
 ...
 * @ingroup discrete
 */
int8_t Modbus_RTU::process_FC15( uint16_t *regs, uint8_t /*u8size*/ )
{
 ...
 * @ingroup register
 */
int8_t Modbus_RTU::process_FC16( uint16_t *regs, uint8_t /*u8size*/ )
{
