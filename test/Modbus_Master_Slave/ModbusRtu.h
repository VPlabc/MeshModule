 ...
 * This includes all the necessary fields to make the Master generate a Modbus_RTU query.
 ...
 * @enum MB_FC
 * @brief
 * Modbus_RTU function codes summary.
 ...
 * @class Modbus_RTU
 * @brief
 * Arduino class library for communicating with Modbus_RTU devices over
...
public:
    void Modbus_Init(uint8_t u8id, Stream& port, uint8_t u8txenpin =0);
    void Modbus_Init1(uint8_t u8id, uint8_t u8serno, uint8_t u8txenpin) __attribute__((deprecated));
    ...
    // Deprecated: Use constructor: "Modbus_RTU m(0,Serial,0)" instead.
    // Modbus_RTU(uint8_t u8id=0, uint8_t u8serno=0, uint8_t u8txenpin=0) __attribute__((deprecated));
